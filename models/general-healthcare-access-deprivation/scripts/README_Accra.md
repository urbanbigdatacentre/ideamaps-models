...
- `General_Healthcare_Access_Accra_V1.ipynb`: Main notebook to run the general healthcare access model.
- `General_Healthcare_Access_Accra_V1.py`: Python script version of the model.
- `Accra/data_inputs/`: Contains all required input data.
- `Accra/data_temp/`: Temporary files created during intermediate processing steps.
...
### 2. **Healthcare Facilities**  
   The healthcare facilities data was extracted from [an official government data set of health care facilities for all of Ghana](https://data.gov.gh/dataset/health-facilities) , the classification for facilities is determined by ownership and type.

   **Format**: CSV (with facility attributes and coordinates)
...
### 1. **Open the notebook:**  
   `General_Healthcare_Access_Accra_V1.ipynb`
...
Further methodological details can be found in the [dataset-interpretability](../Accra-v1/dataset-interpretability.md) documentation.

## 📎 Outputs

Review the outputs in the [output folder](../Accra-v1):
