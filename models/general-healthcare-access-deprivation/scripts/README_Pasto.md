# 🏥 Deploying the General Healthcare Access Deprivation Model – Pasto

This folder contains all required code and input data to assess general healthcare access deprivation for Pasto, Colombia, using a spatial modeling approach based on the isochrones.
...
- `General_Healthcare_Access_Pasto_V1.ipynb`: Main notebook to run the general healthcare access model.
- `General_Healthcare_Access_Pasto_V1.py`: Python script version of the model.
- `Pasto/data_inputs/`: Contains all required input data.
- `Pasto/data_temp/`: Temporary files created during intermediate processing steps.
...
### 1. **Open the notebook:**  
   `General_Healthcare_Access_Pasto_V1.ipynb`
...
Further methodological details can be found in the [dataset-interpretability](../Pasto-v1/dataset-interpretability.md) documentation.

## 📎 Outputs

Review the outputs in the [output folder](../Pasto-v1):
