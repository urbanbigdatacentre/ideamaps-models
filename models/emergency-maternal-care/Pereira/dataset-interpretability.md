...
The focus areas are where defined independently for each city, but in general were the grid cells having values of accessibility very close to the thresholds for each category. There were also considerations for the city of Lano, where the two vulnerable communities that worked within the IDEAMAPS Data Ecosystem are located were closely observed: Doray Karama (Dorayi ward) and Rimin Kebe (Zango ward).
