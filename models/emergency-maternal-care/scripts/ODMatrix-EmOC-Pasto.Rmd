...
origin_pts <- st_read('Pasto/data-temp/healthcare_facilities_emoc.geojson')
dest_pts <- st_read('Pasto/data-temp/pop-grid-pasto-centroids.gpkg')
```
...
```
```{r}
# To prevent gemoetries that are not valide after geocoding, a filter is added. 
origin_pts <- origin_pts[origin_pts$status != 'INVALID_REQUEST', ]
...
write.csv(results_matrix, "Pasto/data-temp/pasto_access.csv", row.names = FALSE)

```

```{r write-data-with-ids}
# Don't forget to write dest_pts as we added a unique (row) id
st_write(origin_pts, 'Pasto/data-temp/healthcare_facilities_emoc_hcfid.geojson', append=F)
