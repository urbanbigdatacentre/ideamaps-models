...
def compute_metric(metric: str, buildings: GeoDataFrame, tessellation: GeoDataFrame, out_path: Path) -> DataFrame:
    building_metrics = ['sdbAre', 'ssbElo', 'stbOri', 'ssbCCD', 'mtbAli', 'mtbNDi', 'ltbIBD', 'ltcBuA', 'mtbNDi3', 'ltbIBD3', 'mtbNDi_log', 'strAli']
    ...
    if metric == 'sdbAre':
        ...
        # TODO: RuntimeWarning: invalid value encountered in scalar divide mean_distances[i] = sub_matrix.sum() / sub_matrix.nnz
        queen_3 = compute_queen_graph(tessellation, 3, out_path)
        values = mm.mean_interbuilding_distance(buildings, queen_3, queen_3)  # Used for SDS (Mdn)
    elif metric == 'ltcBuA':
...
if __name__ == '__main__':
    ...
    metrics = ['sdbAre', 'ssbElo', 'stbOri', 'ssbCCD', 'mtbNDi', 'mtbNDi3', 'mtbNDi_log', 'ltbIBD', 'ltcBuA',
               'sdcAre', 'stcOri', 'sscERI', 'sicCAR', 'mtcWNe', 'mdcAre', 'ltcWRB', 'strAli', 'mtbAli']
